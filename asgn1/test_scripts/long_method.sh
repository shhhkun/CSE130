...
# This tests if httpserver will return '400' Bad Request upon giving an invalid method that is
# strlen(method) > 8 and is in range [a-z][A-Z]
...
for i in {1..10}; do
    ...
    # Method > than expected length so not '501' error
    printf "GETTTTTTTTTTTwijdwa /$infile HTTP/1.1\r\n\r\n" > requestfile
